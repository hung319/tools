...
#================================================================
# Script cài đặt libatk-bridge vào ~/.local và tự động cấu hình shell.
# Script sẽ:
...
# --- Cấu hình ---
# Tên thư viện người dùng yêu cầu
USER_PACKAGE_NAME="libatk-bridge"
# Tên gói thực tế trong apt
APT_PACKAGE_NAME="libatk-bridge2.0-0"
INSTALL_DIR="$HOME/.local"
LIB_DIR="$INSTALL_DIR/lib"

# --- Phần 1: Tải và cài đặt thư viện ---
echo "🚀 Bắt đầu quá trình cài đặt $USER_PACKAGE_NAME vào $INSTALL_DIR"
...
# Tải gói .deb
echo "-> Đang tải gói $APT_PACKAGE_NAME..."
if ! apt-get download "$APT_PACKAGE_NAME" >/dev/null 2>&1; then
    echo "❌ Lỗi: Không thể tải về file .deb của $APT_PACKAGE_NAME. Hãy kiểm tra lại tên gói hoặc kết nối mạng."
    exit 1
fi
...
echo "✅ Hoàn tất cài đặt thư viện $USER_PACKAGE_NAME vào $LIB_DIR."
...
if grep -qF -- "$ENV_LINE" "$CONFIG_FILE"; then
    echo "✔️ Cấu hình LD_LIBRARY_PATH đã tồn tại trong $CONFIG_FILE. Bỏ qua."
else
